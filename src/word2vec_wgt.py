...
def main():
	corpus = []
	n_tr = 200;	#training points - even
	n_ts = 100;	#testing points
	...
	#for doc in corpus_tr:
		...
		#X_tr.append(document_vector_new(model, weights, doc))
	...
	np.save('wgtw2v_train.npy',X_tr)#np.save('wgtw2v_train.npy',X_tr)
	...
	#for doc in corpus_ts:
		...
		#X_ts.append(document_vector_new(model, weights, doc))
	X_ts = np.array(X_ts)
	
	print("#training points : ",len(X_ts))
	#saving test array
	print("saving test array")
	np.save('wgtw2v_test.npy',X_ts)
